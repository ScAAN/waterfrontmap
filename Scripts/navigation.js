...
function print_map_old(){
  // // easy way:
  // window.print()
  // // complex way:
    $('print_container').style.display="block";
    var printimg = getimg3();
    var imgHTML = '<img src="' + printimg + '" style="width:500px"/>';
    //$('myscreenshot').innerHTML = imgHTML;
  }

  function dispimg(img,img2){
    var imgHTML = '<img src="' + img + '" style="width:200px"/>' + '<img src="' + img2 + '" style="width:200px"/>'
    $('myscreenshot').innerHTML = imgHTML;
  }

function getimg3() {
  html2canvas(document.getElementById("test_container"),{backgroundColor:null}).then(function(canvas) {
    tempcan = document.getElementById("myscreenshot").appendChild(canvas);
    tempcan.style.width="100%";
    tempcan.style.height="100%";
    var mapcan  = map.getCanvas().toDataURL();
    var imgcan = canvas.toDataURL();
    //dispimg(mapcan,imgcan)
    tempcan.style.display = "none";
    var img = mergeimgs(imgcan,mapcan,$('my_canvas'));
    $('myscreenshot').innerHTML = '<img src="' + img + '" width="600px"/>';
    return img;
  });
}

function mergeimgs(img1,img2,c){
var ctx=c.getContext("2d");
var imageObj1 = new Image();
var imageObj2 = new Image();
imageObj1.src = img1;
